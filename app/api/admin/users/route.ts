  ...
  try {
    ...
    if (userType === 'resident') {
      if (!name || !address || !phone_number || !nic) {
        return NextResponse.json(
          { error: 'Name, address, phone number, and NIC are required for residents' },
          { status: 400 }
        );
      }
      ...
      if (!department_name || !address || !phone_number) {
        return NextResponse.json(
          { error: 'Department name, address, and phone number are required for department officers' },
          { status: 400 }
        );
      }
    }
    ...
    try {
      if (userType === 'resident') {
        ...
        if (residentError) {
          ...
          return NextResponse.json({ error: 'Failed to create resident profile' }, { status: 500 });
        }
      ...
      return NextResponse.json({
        message: `${userType === 'resident' ? 'Resident' : 'Department officer'} created successfully`,
        user: {
          ...user,
          profile: profileData
        }
