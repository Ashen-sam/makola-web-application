...
export async function GET(request: NextRequest) {
  try {
    ...
    if (allIssues) {
      analytics.statusCounts = allIssues.reduce((acc: StatusCounts, issue) => {
        acc[issue.status] = (acc[issue.status] || 0) + 1;
        return acc;
      }, {});

      analytics.categoryCounts = allIssues.reduce(
        (acc: CategoryCounts, issue) => {
          acc[issue.category] = (acc[issue.category] || 0) + 1;
          return acc;
        },
        {}
      ...
      analytics.priorityCounts = allIssues.reduce(
        (acc: PriorityCounts, issue) => {
          acc[issue.priority] = (acc[issue.priority] || 0) + 1;
          return acc;
        },
        {}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const { issue_ids, status, priority, role } = body;
    ...
    return NextResponse.json({
      message: "Issues updated successfully",
      updated_count: updatedIssues?.length || 0,
    });
  } catch (error) {
    console.error("Admin bulk update error:", error);
    return NextResponse.json(
