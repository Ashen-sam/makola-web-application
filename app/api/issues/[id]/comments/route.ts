...
export async function POST(
  ...
  try {
    ...
    // Validate role
    if (!['resident', 'urban_councilor'].includes(role)) {
    ...
    if (parent_comment_id) {
      ...
      if (parentError || !parentCommentData) {
        return NextResponse.json({ error: 'Parent comment not found or does not belong to this issue' }, { status: 404 });
      }
    ...
    if (issue.user_id && issue.user_id !== user_id) {
      ...
      const notificationMessage = role === "urban_councilor" 
        ? (isReply 
    ...
    if (parentComment && parentComment.user_id && 
      ...
      const replyMessage = role === "urban_councilor"
